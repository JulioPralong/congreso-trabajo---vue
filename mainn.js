...
function Filtros(datos) {
    ...
    var filtroMiembros = datos
    var filtroArray = [];
    filtroMiembros.forEach(miembro => {
        if (checkRepublicano.checked && miembro.party == checkRepublicano.value) {
            //si el checkbock está marcado y la party del miembro es igual al value
            filtroArray.push(miembro);
            //.pusheame (miembro) dentro del FiltroArray
        };
    ...
    if (filtroArray.length == 0) {
        //Si  filtroArray esta  vacio
        tablaDIG(filtroMiembros);
        //mostrar tabla con los miembros
    } else {
        tablaDIG(filtroArray);
        //sino filtramelos
    };
    if (filtroArray.length == 0) {
        //si la longitud del FiltroArray es igual a 0
        filtroArray = filtroMiembros;
        //filtroArray va a ser igual a Filtromiembros
    };
...
function filtroState(data) {
    ...
    var estadosFiltrados = new Set(estados); // no permite valores repetidos
    estadosFiltrados.forEach(state => { //recorrer miembro por miembro
        document.getElementById("fEstados").innerHTML += 
